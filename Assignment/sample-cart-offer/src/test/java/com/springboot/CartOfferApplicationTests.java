...
public class CartOfferApplicationTests {
	...
	@Test
	public void checkApplyOffer() throws Exception {
		ApplyOfferRequest applyOfferRequest =  new ApplyOfferRequest(200,1,1);
		int finalCartValue = applyOffer(applyOfferRequest);
		Assert.assertEquals(finalCartValue, 190);
	}
	...
	public void checkNoOffer() throws Exception {
		...
		int finalCartValue = applyOffer(applyOfferRequest);
		Assert.assertEquals(finalCartValue, 200);
	}

	@Test
	public void checkInvalidOffer() throws Exception {
	...
	public int applyOffer(ApplyOfferRequest applyOfferRequest) throws Exception {
		...
		if (responseCode == HttpURLConnection.HTTP_OK) { //success
			...
			// Parse the response to get the final cart value
			JsonNode jsonNode = mapper.readTree(response.toString());
			return jsonNode.get("cart_value").asInt();
